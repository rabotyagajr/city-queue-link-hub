...
const TimeBankExchange: React.FC<TimeBankExchangeProps> = ({ appointments, onDonate, onViewQR }) => {
    ...
    <div className="mt-6">
      <div className="flex justify-between items-center mb-4">
        ...
        {activeAppointments.length > 0 && (
          <button className="btn-outline" onClick={() => setShowDonateForm(true)}>
            Пожертвовать запись
          </button>
        )}
      </div>
      ...
      {showDonateForm && (
        <div className="card mb-6">
          ...
          <div className="flex justify-end gap-2">
            ...
            <button className="btn-primary" onClick={handleDonate} disabled={!selectedAppointment}>
              Пожертвовать
            </button>
          </div>
        </div>
